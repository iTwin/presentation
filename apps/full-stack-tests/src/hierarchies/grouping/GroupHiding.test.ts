  ...
  describe("Grouping nodes' hiding", () => {
    ...
    describe("Base class grouping", () => {
      ...
      it("hides base class groups when there're no siblings", async () => {
        const { imodel, ...keys } = await buildIModel("hides base class groups when there're no siblings", async (builder) => {
      ...
      it("hides base class groups when there's only 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("hides base class groups when there's only 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
      ...
      it("doesn't hide base class groups when there are siblings", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide base class groups when there are siblings", async (builder) => {
      ...
      it("doesn't hide base class groups when there's more than 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide base class groups when there's more than 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
          const childSubject2 = insertSubject({ builder, codeValue: "A2", parentId: IModel.rootSubjectId });
          return { childSubject1, childSubject2 };
        });
    ...
    describe("Class grouping", () => {
      ...
      it("hides class groups when there're no siblings", async () => {
        const { imodel, ...keys } = await buildIModel("hides class groups when there're no siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
          const childSubject2 = insertSubject({ builder, codeValue: "A2", parentId: IModel.rootSubjectId });
          return { childSubject1, childSubject2 };
        });
      ...
      it("hides class groups when there's only 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("hides class groups when there's only 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
          return { childSubject1 };
        });
      ...
      it("doesn't hide class groups when there are siblings", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide class groups when there are siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
          const childPartition2 = insertPhysicalPartition({ builder, codeValue: "B1", parentId: IModel.rootSubjectId });
          return { childSubject1, childPartition2 };
        });
      ...
      it("doesn't hide class groups when there's more than 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide class groups when there's more than 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId });
          const childSubject2 = insertSubject({ builder, codeValue: "A2", parentId: IModel.rootSubjectId });
          return { childSubject1, childSubject2 };
        });
    ...
    describe("Label grouping", () => {
      ...
      it("hides label groups when there're no siblings", async () => {
        const { imodel, ...keys } = await buildIModel("hides label groups when there're no siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
      ...
      it("hides label groups when there's only 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("hides label groups when there's only 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
          return { childSubject1 };
        });
      ...
      it("doesn't hide label groups when there are siblings", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide label groups when there are siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
      ...
      it("doesn't hide label groups when there's more than 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide label groups when there's more than 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
          const childSubject2 = insertSubject({ builder, codeValue: "A2", parentId: IModel.rootSubjectId, userLabel: groupName });
          return { childSubject1, childSubject2 };
        });
    ...
    describe("Properties grouping", () => {
      ...
      it("hides property groups when there're no siblings", async () => {
        const { imodel, ...keys } = await buildIModel("hides property groups when there're no siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
          const childSubject2 = insertSubject({ builder, codeValue: "A2", parentId: IModel.rootSubjectId, userLabel: groupName });
          return { childSubject1, childSubject2 };
        });
      ...
      it("hides property groups when there's only 1 grouped node", async () => {
        const { imodel, ...keys } = await buildIModel("hides property groups when there's only 1 grouped node", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
          return { childSubject1 };
        });
      ...
      it("doesn't hide property groups when there are siblings", async () => {
        const { imodel, ...keys } = await buildIModel("doesn't hide property groups when there are siblings", async (builder) => {
          const childSubject1 = insertSubject({ builder, codeValue: "A1", parentId: IModel.rootSubjectId, userLabel: groupName });
