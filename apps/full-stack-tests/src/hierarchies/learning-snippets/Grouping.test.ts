    ...
    describe("Grouping", () => {
      ...
      describe("By label", () => {
        ...
        it("merges by label", async () => {
          const { imodel } = await buildIModel("merges by label", async (builder) => {
            const category = insertSpatialCategory({ builder, codeValue: "Category" });
            const model = insertPhysicalModelWithPartition({ builder, codeValue: "Model" });
      ...
      describe("By class", () => {
        it("groups by node's class", async () => {
          const { imodel } = await buildIModel("groups by node's class", async (builder) => {
        ...
        it("groups by base classes", async () => {
          const { imodel } = await buildIModel("groups by base classes", async (builder) => {
            const drawingCategory = insertDrawingCategory({ builder, codeValue: "Example drawing category" });
            const spatialCategory = insertSpatialCategory({ builder, codeValue: "Example spatial category" });
            return { spatialCategory, drawingCategory };
          });
      ...
      describe("By properties", () => {
        it("groups by property value", async () => {
          const { imodel } = await buildIModel("groups by property value", async (builder) => {
        ...
        it("groups by property value ranges", async () => {
          const { imodel } = await buildIModel("groups by property value ranges", async (builder) => {
      ...
      it("creates multi-level grouping hierarchy", async () => {
        const { imodel } = await buildIModel("creates multi-level grouping hierarchy", async (builder) => {
      ...
      describe("Customization options", () => {
        it("doesn't return grouping node if there's only one grouped instance and `hideIfOneGroupedNode = true`", async () => {
          const { imodel } = await buildIModel(
            "doesn't return grouping node if there's only one grouped instance and `hideIfOneGroupedNode = true`",
            async (builder) => {
        ...
        it("doesn't return grouping node if it has no siblings and `hideIfNoSiblings = true`", async () => {
          const { imodel } = await buildIModel("doesn't return grouping node if it has no siblings and `hideIfNoSiblings = true`", async (builder) => {
            insertRepositoryLink({ builder, repositoryLabel: "Example link 1" });
            insertRepositoryLink({ builder, repositoryLabel: "Example link 2" });
          });
          const imodelAccess = createIModelAccess(imodel);
        ...
        it("sets auto-expand flag on grouping nodes when `autoExpand = true`", async () => {
          const { imodel } = await buildIModel("sets auto-expand flag on grouping nodes when `autoExpand = true`", async (builder) => {
            insertRepositoryLink({ builder, repositoryLabel: "Example link 1" });
            insertRepositoryLink({ builder, repositoryLabel: "Example link 2" });
          });
          const imodelAccess = createIModelAccess(imodel);
