...
import { BeDuration, Id64, Id64String } from "@itwin/core-bentley";
...
describe("Unified Selection", () => {
  ...
  describe("Hiliting selection", () => {
    ...
    const instances = {
      ...
      transientElement: {
        key: { className: TRANSIENT_ELEMENT_CLASSNAME, id: Id64.fromLocalAndBriefcaseIds(123, 0xffffff) },
      },
    };
    ...
    beforeEach(() => {
      ...
      imodel.selectionSet.add(Id64.fromLocalAndBriefcaseIds(1, 1));
    });
    ...
    it("hilites subject", async () => {
      ...
      await waitForAllAsyncs([handler, Presentation.selection.getToolSelectionSyncHandler(imodel)!]); // eslint-disable-line @itwin/no-internal
...
const waitForAllAsyncs = async (handlers: Array<{ pendingAsyncs: Set<string> }>) => {
