...
/**
 * A union of prop types for selecting a value in ECSQL query.
 * @see [[createValueSelector]]
 * @beta
 */
...
/**
 * Create a selector combined of multiple value selectors in a form of a JSON array. This allows handling results
 * of each value selector individually when parsing query result.
 *
 * Example result: `json_array(VALUE_SELECTOR_1, VALUE_SELECTOR_2, ...)`.
 *
 * Optionally, the function also accepts a `checkSelector` argument, which can be used to make the query return
 ...
 * @beta
 */
export function createConcatenatedValueSelector(selectors: ValueSelectClauseProps[], checkSelector?: string) {
  if (selectors.length === 0) {
    return "''";
  }
  const combinedSelectors = `json_array(${selectors.map(createValueSelector).join(", ")})`;
  if (checkSelector) {
...
/**
 * Creates an ECSQL selector for a value based on given props.
 * @beta
 */
export function createValueSelector(props: ValueSelectClauseProps): string {
  if (ValueSelectClauseProps.isPropertySelector(props)) {
    if (props.specialType) {
      ...
      return createValueSelector({ selector: valueSelector, type: typeOverride, nullValueResult: props.nullValueResult });
    }
  ...
  if (ValueSelectClauseProps.isPrimitiveValueSelector(props)) {
    const typedSelector = `
      json_object(
