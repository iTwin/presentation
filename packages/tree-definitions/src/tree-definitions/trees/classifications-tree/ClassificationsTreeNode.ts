...
export namespace ClassificationsTreeNode {
  ...
  export const isClassificationTableNode = (
    node: Pick<HierarchyNode, "extendedData">,
  ): node is NonGroupingHierarchyNode & { key: InstancesNodeKey } => node.extendedData?.type === "classification-table";

  /** Checks if the given node represents a `ClassificationSystems.Classification` element. */
  export const isClassificationNode = (
    node: Pick<HierarchyNode, "extendedData">,
  ): node is NonGroupingHierarchyNode & { key: InstancesNodeKey } => node.extendedData?.type === "classification";
