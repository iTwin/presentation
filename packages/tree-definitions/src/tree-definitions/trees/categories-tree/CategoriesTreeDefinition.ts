...
import type { CachedCategoryInfo, CategoriesTreeIdsCache } from "./CategoriesTreeIdsCache.js";
...
export class CategoriesTreeDefinition implements HierarchyDefinition {
  ...
  public constructor(props: CategoriesTreeDefinitionProps) {
    ...
    this.#excludedClasses =
      this.#hierarchyConfig.elements.nodes === "include" ? this.#hierarchyConfig.elements.excludedClasses : undefined;
  }
  ...
  public preProcessNode: NodePreProcessor = async ({ node }) => {
    if (CategoriesTreeNodeInternal.isRawCategoryNode(node)) {
      return {
        ...node,
        extendedData: { ...node.extendedData, modelIds: parseIdsSelectorResult(node.extendedData.modelIds) },
      };
    }
    return node;
  };

  private static extendPathWithElement(elementNode: {
  ...
  private static getInheritedParentElementsPath(
    parentNode: NonNullable<Props<NodePostProcessor>["parentNode"]>,
  ): ParentElementsPath {
    if (
      CategoriesTreeNodeInternal.isElementClassGroupingNode(parentNode) ||
      CategoriesTreeNodeInternal.isCategoryNode(parentNode)
    ) {
  ...
  private assignParentElementsPath({
    node,
    parentNode,
  }: Pick<Props<NodePostProcessor>, "node" | "parentNode">): ProcessedHierarchyNode {
    if (CategoriesTreeNodeInternal.isRawCategoryNode(node)) {
    ...
    if (
      CategoriesTreeNodeInternal.isRawElementNode(node) ||
      CategoriesTreeNodeInternal.isRawElementClassGroupingNode(node)
    ) {
  ...
  public postProcessNode: NodePostProcessor = async ({ node, parentNode }) => {
    ...
    const modelElementsMap = new Map<
      ModelId,
      { elementIds: Set<ElementId>; childrenWhichAreParents: Set<ElementId> }
    >();
            ...
            ...(this.#hierarchyConfig.elements.nodes === "include"
              ? [
                  ...
                  },
                  {
                    ...
                    definitions: async (requestProps: DefineInstanceNodeChildHierarchyLevelProps) =>
                      this.createGeometricModelChildrenQuery(requestProps),
                  },
                ]
              : []),
            // When sub-categories and elements are not shown, category will never have children
            ...(this.#hierarchyConfig.subCategories.nodes === "exclude" &&
            this.#hierarchyConfig.elements.nodes === "exclude"
              ? []
              : [
                  {
                    parentInstancesNodePredicate: this.#categoryClass,
                    definitions: async (requestProps: DefineInstanceNodeChildHierarchyLevelProps) =>
                      this.createCategoryChildrenQuery(requestProps),
                  },
                ]),
  ...
  private async createGeometricModelChildrenQuery(
    props: DefineInstanceNodeChildHierarchyLevelProps,
  ...
  private async createDefinitionContainersAndCategoriesQuery(
    ...
    if (!categories || categories.length > 0) {
      hierarchyDefinitionPromises.push(
        categories
          ? this.createCachedTopMostCategoriesQuery({
              categories,
              instanceFilter,
              createSelectClause,
              createFilterClauses,
  ...
  private async createDefinitionContainerNodeSelectClause(
    createSelectClause: DefineHierarchyLevelProps["createSelectClause"],
  ): Promise<string> {
    return createSelectClause({
      ecClassId: { selector: ECSql.createRawPropertyValueSelector("this", "ECClassId") },
      ecInstanceId: { selector: "this.ECInstanceId" },
      nodeLabel: { of: { classAlias: "this", className: CLASS_NAME_DefinitionContainer } },
          ...
          ecsql: `
            SELECT
              ${await createSelectClause({
                ecClassId: { selector: "this.ECClassId" },
                ecInstanceId: { selector: "this.ECInstanceId" },
                nodeLabel: { of: { classAlias: "this", className: CLASS_NAME_SubCategory } },
  ...
  private async createElementNodeSelectClause({
    ...
    const selectClause = await createSelectClause({
      ecClassId: { selector: "this.ECClassId" },
      ecInstanceId: { selector: "this.ECInstanceId" },
      nodeLabel: { of: { classAlias: "this", className: this.#categoryElementClass } },
      hasChildren: {
        selector: `
          IFNULL(
            (
  ...
  private async createCategoryNodeSelectClause({
    ...
    return createSelectClause({
      ecClassId: { selector: "this.ECClassId" },
      ecInstanceId: { selector: "this.ECInstanceId" },
      nodeLabel: { of: { classAlias: "this", className: this.#categoryClass } },
          ...
          ecsql: `
            SELECT
              ${await this.createCategoryNodeSelectClause({
                createSelectClause,
                hasChildren: true,
                extendedData: { modelIds: { selector: createIdsSelector([parentNode.extendedData.modelId]) } },
              })}
            FROM ${categoryInstanceFilterClauses.from} this
...
function createInstanceKeyPathsFromInstanceLabel(
  ...
  const { idsCache, label, viewType, labelsFactory, limit, imodelAccess, componentId, componentName, hierarchyConfig } =
    props;
      ...
      mergeMap(async ({ definitionContainers, categories }) => {
        ...
        const [
          ...
          [
            ...
            CLASS_NAME_SubCategory,
            elementClass,
            ...(definitionContainers.length > 0 ? [CLASS_NAME_DefinitionContainer] : []),
          ].map(async (className) =>
