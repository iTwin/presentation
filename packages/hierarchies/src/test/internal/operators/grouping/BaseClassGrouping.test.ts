...
import { ECClass, IMetadataProvider } from "../../../../hierarchies/ECMetadata";
...
describe("BaseClassGrouping", () => {
  ...
  describe("createBaseClassGroupsForSingleBaseClass", async () => {
    it("doesn't group if provided ECClass is not in the nodes' grouping base class list", async () => {
      ...
      expect(await baseClassGrouping.createBaseClassGroupsForSingleBaseClass(nodes, ecClass, new BaseClassChecker(metadataProvider, 1000))).to.deep.eq({
    ...
    it("doesn't call getClass when it was called before with the same className", async () => {
      ...
      await baseClassGrouping.createBaseClassGroupsForSingleBaseClass(nodes, ecClass, new BaseClassChecker(metadataProvider, 1000));
  ...
  describe("createBaseClassGroupingHandlers", () => {
    it("creates base class grouping handlers from the provided fullClassNames", async () => {
      ...
      const result = await baseClassGrouping.createBaseClassGroupingHandlers(metadataProvider, nodes, new BaseClassChecker(metadataProvider, 1000));
