...
describe("TypedPrimitiveValue", () => {
  describe("create", () => {
    it("returns correct result when primitiveType is compatible with primitiveValue", () => {
      ...
      expect(TypedPrimitiveValue.create(1, "Integer")).to.deep.eq({
        value: 1,
        type: "Integer",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create(1, "Double")).to.deep.eq({
        value: 1,
        type: "Double",
        extendedType: undefined,
        koqName: undefined,
      });
      expect(TypedPrimitiveValue.create("0x11", "Id")).to.deep.eq({
        value: "0x11",
        type: "Id",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create(true, "Boolean")).to.deep.eq({
        value: true,
        type: "Boolean",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create("someValue", "String")).to.deep.eq({
        value: "someValue",
        type: "String",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create("someValue", "DateTime")).to.deep.eq({
        value: "someValue",
        type: "DateTime",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create(1, "DateTime")).to.deep.eq({
        value: 1,
        type: "DateTime",
        extendedType: undefined,
      });
      const date = new Date();
      expect(TypedPrimitiveValue.create(date, "DateTime")).to.deep.eq({
        value: date,
        type: "DateTime",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create({ x: 1, y: 2 }, "Point2d")).to.deep.eq({
        value: { x: 1, y: 2 },
        type: "Point2d",
        extendedType: undefined,
      });
      expect(TypedPrimitiveValue.create({ x: 1, y: 2, z: 3 }, "Point3d")).to.deep.eq({
        value: { x: 1, y: 2, z: 3 },
        type: "Point3d",
        extendedType: undefined,
      });
