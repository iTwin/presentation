...
### Private value requirements

The provider-owned content definition records the raw columns to select from the iModel, deduplicated across the fields and external inputs that read them. These private value requirements separate two concerns:
...
Requirement kinds mirror the SQL-backed field kinds:

- A property requirement selects a real EC property column. Its private ID equals the property field's _base_ ID (source class + property name + path, with no fork suffix), so every fork/override variant of the same property collapses to one read.
- A calculated requirement selects the result of an ECSQL expression and uses the calculated field ID privately.

External fields have no SQL requirement of their own — they are populated out-of-band.

The content definition privately maps each SQL-backed field ID to its requirement. At value loading, one loaded column is written to every mapped field. Neither this mapping nor the requirement set is exposed on the public descriptor.
...
#### Nested content: `applyRecursively`

An iModel fields provider may opt in via `IModelFieldsProvider.applyRecursively: true` to have its contribution additionally applied on every **nested anchor** — a concrete related-instance class surfaced by _any_ resolved related-properties path, from any provider (including itself). This mirrors the native `ContentModifier.applyOnNestedContent` rule attribute (e.g., the native aspects fields provider pulling in "properties of properties" — related-instance aspects reached through a first related instance, such as `Wall` → `WallType` → `WallType`'s own aspects).

Design decisions:

- **Anchor derivation.** For a producing declaration, the anchor step(s) are: the path's final step when the declaration's `properties` is omitted (all properties of the final class are loaded, so the final class is the only reachable "surface"); when `properties` is given, every step whose target selects `"all"` or `{ exclude: [...] }`. Steps with `"none"` or `{ include: [...] }` do not anchor nested contributions because they intentionally expose no properties or only a selected subset.
- **Provider invocation with synthesized targets.** On a nested anchor, `getContribution` is invoked with a synthesized `{ primaryClass: anchorClassName }` target — no `instanceIds` / `instanceFilter`. Providers must treat the absence of those members as "not scoped to specific instances", exactly as for an omitted direct target.
- **Full-path resolution anchored at the original target — not the anchor class alone.** This is the critical correctness requirement. A nested declaration's concrete suffix is always resolved as part of the **full path from the original content target** (concrete prefix up to and including the anchor step, plus the suffix), never as an independent query rooted at the anchor class. Resolving from the anchor alone would silently drop the original target's `instanceIds` / `instanceFilter` scoping and leak sibling instances — e.g., if `Wall1` and `Wall2` both reach the shared class `WallType`, but each references a _different_ concrete `WallType`-derived aspect subclass, resolving the nested aspect declaration from `WallType` alone (unscoped) would return _both_ aspect subclasses for _both_ walls' content sources, even though `Wall1`'s actual aspect instance is only ever the first kind. Anchoring the query at the original target's instance scope keeps each wall's resolved paths limited to the aspect classes actually reachable through its own related `WallType` instance.
- **Recursion and cycle guard.** Nested application is recursive and unbounded in depth — an anchor surfaced by a nested contribution can itself receive nested contributions from any opted-in provider, including the same one. Termination is data-driven: a nested declaration whose full path matches no instances resolves to zero concrete paths and contributes nothing further. As a guard against cyclic instance graphs (e.g. a self-referential relationship), a given `(providerId, anchorClassName)` pair is applied at most once per expansion branch (siblings in the expansion tree do not share the guard — only descendants of the branch where the pair was first applied are blocked).
- **Custom `resolve` callbacks are skipped as nested suffixes.** A declaration with a custom `resolve` callback cannot be composed into a full path, because its resolution logic runs from the target class alone and has no way to honor an arbitrary prefix. Such declarations are therefore never applied as nested suffixes. They are, however, still eligible to _act as anchors_ for other providers' nested contributions when their own resolved path lands them as a parent in the expansion.
- **Only `relatedProperties` participate — not `calculatedFields`.** Nested application exists solely to pull in new related targets; only `relatedProperties` declarations introduce new related targets. `calculatedFields` are never applied on nested content (referenced `categories` are pulled in as needed, same as for base groups).
- **Deterministic, serializable ordering.** Base groups (declarations applied directly on the source's own target) are ordered first (by provider, then declaration order), followed by nested groups in breadth-first expansion order (shallower anchors first). The array is otherwise deterministic and stable across runs for the same inputs and provider set, keeping serialized `ContentSource`s cacheable (byte-stable).

See `IModelFieldsProvider.applyRecursively` and `ResolvedDeclarationGroup.nested` for the full API-level contract, and `ResolveContentSources.ts` for the implementation.
...
Final field list:
  ...
  Input resolution:
    - "pumpName" input → private property requirement for Pump.Name
    - The requirement's column is queried regardless of whether any output field references it,
...
2. **Static declarations are self-describing** — The provider declares exactly what it needs (`className`, `propertyName`, `path?`). The pipeline records a private value requirement for it. The provider never needs to find anything in the descriptor.
...
interface InputPropertyDeclaration {
  ...
  related?: {
    // omit for a direct property input
...
**Input column rule:** For each declared input property, the pipeline adds a private property requirement unconditionally, regardless of whether any output field references the same column. Because requirements are independent of the public field set, removing an output field cannot remove an input column. The provider declares what it needs and receives the decoded value in `inputValues`.
...
**Example:** An external fields provider declares `inputs: { externalId: { className: "BisCore:Element", propertyName: "CodeValue" } }`. It declares `fields: [{ id: "externalName", ... }, { id: "externalStatus", ... }]`. The pipeline records a private requirement for `CodeValue` so the column is queried without creating a field. During Stage 4, the provider's `getValues` function reads `item.inputValues.externalId` from each item, calls an external service once with all values, and returns `externalName` and `externalStatus` for each item in the batch.
