...
- **Anchor derivation.** For a producing declaration, the anchor step(s) are: the path's final step when the declaration's `properties` is omitted (all properties of the final class are loaded, so the final class is the only reachable "surface"); when `properties` is given, every step whose `target.select === "all"` (a step whose target class properties are fully loaded is a legitimate surface for further nesting — steps with `"none"` or a partial `include`/`exclude` selection are not, since they weren't meant to expose the whole class).
