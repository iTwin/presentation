...
1. All components that we own are compatible with the new system as the first class citizen.
   ...
   - The [viewWithUnifiedSelection](https://www.itwinjs.org/reference/presentation-components/viewport/viewwithunifiedselection/) HOC was deprecated in favor of the `enableUnifiedSelectionSyncWithIModel` provided by this package. The API is quite a bit different, but it's more clear about what it does and is more flexible in how it can be used.

     While the `viewWithUnifiedSelection` HOC had to be applied on a viewport component, the new `enableUnifiedSelectionSyncWithIModel` function should be called in the top-level component that has access to the iModel connection. The synchronization happens on the `IModelConnection` level, so all viewports associated with the same `IModelConnection` are automatically synchronized. This also means that there is no need to wrap any viewport components with a HOC - just render them as they are. See the [iModel selection synchronization with unified selection](./SyncWithIModelConnection.md) learning page for more information and example for using it in a React app.
